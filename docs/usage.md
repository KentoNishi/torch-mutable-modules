...
First, import `torch-mutable-modules`.
...
from torch_mutable_modules import get_fast_shifts, pitch_shift
```

## What's included
`torch-mutable-modules` includes the following:
...
See [example.py](https://github.com/KentoNishi/torch-mutable-modules/blob/master/example.py) to see an example of `torch-mutable-modules` in action!
